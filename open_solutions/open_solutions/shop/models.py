...
class ShopProduct(models.Model):
    product = models.ForeignKey(
        "Product", on_delete=models.CASCADE, related_name="shop_product"
    )
    shop = models.ForeignKey("Shop", on_delete=models.CASCADE)
    quantity = models.IntegerField("Количество", default=0)
    price = models.DecimalField("Цена", max_digits=10, decimal_places=2, default=0)
    price_rrc = models.DecimalField(
        "Рекомендованная розничная цена", max_digits=10, decimal_places=2, default=0
    )
    description = models.TextField("Описание", blank=True)
...
class ProductParameter(models.Model):
    shop_product = models.ForeignKey(
        "ShopProduct", on_delete=models.CASCADE, verbose_name="Информация о продукте"
    )
...
class Order(models.Model):
    ...
    datatime = models.DateTimeField("Дата и время оформления заказа", auto_now_add=True)
...
class OrderItem(models.Model):
    ...
    status = models.CharField(
        "Статус заказа", max_length=20, choices=Status.choices, default=Status.NEW
    )
    quantity = models.IntegerField("Количество", default=0)
